...
printf %s "sudo docker run -d --name=jellyfin --restart unless-stopped --net=host -e PUID=$uid -e PGID=$gid -e TZ=Asia/Shanghai -v $docker/jellyfin:/config `cat $docker/note/$name\2.sh` >> $docker/jellyfin/jellyfin.sh"
if [ -c `ls /dev/dri | grep renderD128` ];then
